...
tables.forEach(table =>{
  ...
  if(Array.isArray(table.table)) {
    ...
    table.table.forEach((tr)=> {
     ...
     const cypherName = titleCase(tr.entry.toLowerCase().trim())
     .split("(").join("")
     .split(")").join("")
     const cypherFile = titleCase(tr.entry.toLowerCase().trim())
     .split("(").join("")
     .split(")").join("")
     .split(' ').join('-')
     content.push(`| ${numCol} | [[${cypherFile}\\|${cypherName}]] |`)
    })
  }
